...
ScreenshotManager::ScreenshotManager(Ogre::RenderWindow* pRenderWindow, int gridSize, Ogre::String fileExtension, bool overlayFlag)
{
    ...
    //create PixelBox
    mData = new uint8[(mWindowWidth * mGridSize) * (mWindowHeight * mGridSize) * 3];
    mFinalPicturePB = PixelBox(mWindowWidth * mGridSize,mWindowHeight * mGridSize,1,PF_B8G8R8,mData);
...
void ScreenshotManager::makeScreenshot(Ogre::Camera* camera, Ogre::String fileName) const
{
    ...
    bool enableOverlayFlag = myApplication::getSingletonPtr()->getDefaultViewport()->getOverlaysEnabled();
    ...
    if(mDisableOverlays && enableOverlayFlag)
        MyApplication::getSingletonPtr()->getDefaultViewport()->setOverlaysEnabled(false);
    ...
    else
    {
        ...
        { 
            ...
            MyApplication::getSingletonPtr()->getOgreRoot()->clearEventTimes();
            mRT->update();        //render
